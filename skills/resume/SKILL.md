...
- `resume-template-editable.html`：可编辑单页模板起点（含「自动一页」开关）；
...
3. 按“模板选择矩阵”选择模板、页数和风格；内容能在正常字号下形成完整单页时优先单页，不因模板预设第二张页面而扩页。单页内容略微超出时，先用模板自带的「自动一页」压缩（压间距→压行距→二分字号，正文字号不低于 9pt），压不下来或可读性受损再改为双页。如果资源缺失，启用后备模板并说明限制。
4. 输出 HTML 文件，保持文字和布局可编辑；工具栏放在页面外并在打印时隐藏。
5. 把 print-preview 作为导出前强制 QA：打开或模拟打印预览，检查分页、双栏、照片比例、长文本溢出、中文字体、打印边距和 A4 纸张效果；单页模板可借助「自动一页」开关与 A4 溢出提示，开启/关闭对比分页是否稳定；再按 [A4 页面平衡与视觉密度 QA](references/page-balance-qa.md) 测量每页内容占用并保存整页截图。发现问题先修复 HTML/CSS 再交付。
...
- 单页内容略微超出时先启用模板「自动一页」（压间距→压行距→二分字号，下限 9pt）；仍放不下或压缩已损害可读性时，改为双页而不是继续把字号缩到不可读；
...
- 单页模板默认排版与既有模板一致；开启「自动一页」后按「压间距 → 压行距 → 二分字号」自动压缩，正文字号不低于 9pt；
...
PDF 不是简单“能保存”即可。导出或指导用户导出时，按以下标准验收。本机装有 Edge/Chrome 时，可用零依赖脚本 [scripts/export-resume-pdf.mjs](../../scripts/export-resume-pdf.mjs) 自动导出（`npm run export:pdf -- <html路径> --out <pdf>`，`preferCSSPageSize` 让 PDF 纸张跟随模板 `@page`，不受浏览器默认纸型影响）；浏览器打印另存为 PDF 作为兜底。
